...
def _to_read(project: Projects, done: int, total: int) -> ProjectsRead:
    return ProjectsRead(
        **project.model_dump(),
        points_done=done,
        points_total=total,
...
def get_projects(session: Session = Depends(get_session)):
    ...
    rows = session.exec(
        select(Projects, points_done, points_total)
    ...
    return [_to_read(project, done, total) for project, done, total in rows]
